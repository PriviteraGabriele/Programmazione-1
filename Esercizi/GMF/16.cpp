    ...
    for(int i = 0; i<n; i++){
        ...
        for(int j = 0; j<m; j++){
            if(S[i][j]){
                ...
                //cout << "S_lunghezza = " << (*S[i][j]).length() << endl;
                //cout << "B_lunghezza = " << B[j] << endl;
                if((*S[i][j]).length() <= B[j]){
                    counter++;
...
int main(){
    ...
    cout << "L’indice della riga in S con il maggior numero di stringhe aventi lunghezza minore o uguale del corrispondente numero nello array B è : " << esercizio16(S, n, m, B) << endl;
}